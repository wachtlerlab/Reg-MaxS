...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 9 and sys.argv[1] == "saveData":
        ...
        reflections = np.array([int(x) for x in eval(sys.argv[7])])
        rotations = np.array([float(x) for x in eval(sys.argv[8])])
        ...
        raise ValueError(
            "I could not understand your input! Please use as one of the following:\n"
            "Use case 1: Saving densities:\n----\n"
            "python {fle} saveData arg1 arg2 arg3 arg4 arg5 arg6 arg7\n\n"
            "arg1: spatial discretization size: pixel size of output image\n"
            "arg2: path of the parameter file used for RegMaxS/N\n"
            "arg3: path of an SWC file of the set, which will be at the center of density plots\n"
            "arg4: path of a temporary file that will contain density data (without extention)\n"
            "arg5: standard deviation of the Gaussian kernel used to smooth density\n"
            "arg6: string representation of 3 member tuple, one for each dimension, "
            "indicating whether to mirror about the centroid along this axis\n"
            "arg7: string representation of 3 member tuple, one for each dimension, "
            "indicating the number of degrees by which all SWCs must be rotated about this dimension\n\n"
            "Use case 2: Plotting densities from a single compressed .npz file\n----\n"
            "python {fle} savePlotsSingle <compressed .npz Data file generated by saveData> "
            "<label> <output directory>\n\n"
            "Output will be written to the files `<label>12.png` (view of dimensions 1 and 2) and "
            "`<label>13.png` (view of dimensions 1 and 3) in the <output directory>\n\n"
            "Use sase 3: Plotting densities from a multiple compressed .npz files in a directory\n----\n"
